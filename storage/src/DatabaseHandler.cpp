...
DatabaseHandler::DatabaseHandler(const std::string& db_path)
    : cleanup_timer(
          new Timer(std::bind(&DatabaseHandler::perform_cleanup, this))) {
    open_and_prepare(db_path);
    cleanup_timer->start();
}

void DatabaseHandler::perform_cleanup() {
...
sqlite3_stmt* DatabaseHandler::prepare_statement(const std::string& query) {
    const char* pzTest;
    sqlite3_stmt* stmt;
...
void DatabaseHandler::open_and_prepare(const std::string& db_path) {
...
bool DatabaseHandler::retrieve(const std::string& pubKey,
