...
bt_value store::to_bt() const {
    ...
    if (subaccount) {
        d["subaccount"] = subaccount->token.sview();
        d["subaccount_sig"] = util::view_guts(subaccount->signature);
    }
    if (signature)
...
bt_value delete_msgs::to_bt() const {
    ...
    if (pubkey_ed25519)
        ret["pubkey_ed25519"] = std::string_view{
                reinterpret_cast<const char*>(pubkey_ed25519->data()), pubkey_ed25519->size()};
    if (subaccount) {
        ret["subaccount"] = subaccount->token.sview();
        ret["subaccount_sig"] = util::view_guts(subaccount->signature);
    }
    return ret;
}
